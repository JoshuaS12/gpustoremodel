...
export default class Login extends Component {
    ...
    onLoginClick = async () => {
        ...
        if (body.length > 0) { // checks if there's an item in the array, without .length > 0 it always gives success
            // login success
            this.setState({message: <span className="text-success">Successfully logged in!</span>})
            } else {
                //login error
                this.setState({message: <span className="text-danger">Login is invalid, please try again.</span>})
            }
        }
    };